...
void artexp_delete_expression(ArithmeticExpression *src) {
    ArithmeticExpressionStack *stack;

    stack = src->prefixStack;
    while (stack->top != NULL) {
        ArithmeticExpressionElement *top = stack->top;
        stack->top = stack->top->next;
        free(top);
    }

    stack = src->postfixStack;
    while (stack->top != NULL) {
        ArithmeticExpressionElement *top = stack->top;
        stack->top = stack->top->next;
        free(top);
...
int artexp_evaluate_postfix(char *postfix) {
    ...
    for (int i = 0; i < len; i++) {
        if (isalnum(postfix[i])) {
            ...
            switch (postfix[i]) {
                case '+':
                    artexpstack_push_node(stack, *artexpstack_create_node(val2 + val1, 'a'));
                    break;
                case '-':
                    artexpstack_push_node(stack, *artexpstack_create_node(val2 - val1, 'b'));
                    break;
                case '*':
                    artexpstack_push_node(stack, *artexpstack_create_node(val2 * val1, 'c'));
                    break;
                case '/':
                    artexpstack_push_node(stack, *artexpstack_create_node(val2 / val1, 'd'));
                    break;
                case '^':
                    artexpstack_push_node(stack, *artexpstack_create_node((int) pow(val2, val1), 'e'));
                    break;
            }
        }
    }
...
int artexp_evaluate_prefix(char *prefix) {
    ...
    for (int i = len - 1; i >= 0; i--) {
        if (isalnum(prefix[i])) {
            ...
            switch (prefix[i]) {
                case '+':
                    artexpstack_push_node(stack, *artexpstack_create_node(val1 + val2, 'a'));
                    break;
                case '-':
                    artexpstack_push_node(stack, *artexpstack_create_node(val1 - val2, 'b'));
                    break;
                case '*':
                    artexpstack_push_node(stack, *artexpstack_create_node(val1 * val2, 'c'));
                    break;
                case '/':
                    artexpstack_push_node(stack, *artexpstack_create_node(val1 / val2, 'd'));
                    break;
                case '^':
                    artexpstack_push_node(stack, *artexpstack_create_node((int) pow(val1, val2), 'e'));
                    break;
            }
        }
    }
...
ArithmeticExpressionStack * artexp_infix_to_prefix_stack(char *infix) {
    ...
    ArithmeticExpressionStack *stack = artexp_prefix_to_stack(prefix);
    return stack;
}
...
ArithmeticExpressionStack * artexp_infix_to_postfix_stack(char *infix) {
    ...
    for (int i = 0; i < len; i++) {
        ...
        ArithmeticExpressionNode *newNode = artexpstack_create_node(infix[i], infix[i]);
